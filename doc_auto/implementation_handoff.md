...
4. `bash scripts/build-release.sh`、`LOCAL_RELEASE_DIR=release bash scripts/install.sh`、`LOCAL_RELEASE_DIR=release bash scripts/update-check.sh`：通过。
5. served web/runtime 与 served desktop-preview smoke checks：通过。
6. `bash scripts/validate-release.sh`：通过，已覆盖 release dir validation、release-site parity、install hook 与 local update readiness。
7. `tests.test_enva_migration_validation`：通过，已覆盖 target override、release smoke、mock release API 与 migration readiness 报告。
8. Pages workflow 与 release workflow 现在都包含 migration validation 基线，不再只有 release 路径跑完整 Python trio。
