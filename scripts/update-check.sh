...
main() {
  ...
  if [[ -n "$LOCAL_RELEASE_DIR" && -f "$LOCAL_RELEASE_DIR/$asset_name" ]]; then
    ...
    echo "Manifest: $LOCAL_RELEASE_DIR/release-manifest.json"
    echo "Migration ready: yes"
    return 0
  fi

  response="$(curl -fsSL "$(rwc_release_api_url)")"
  python3 - <<'PY' "$response" "$asset_name" "$RWC_CURRENT_TAG" "$RWC_UPDATE_REQUIRE_MANIFEST" "$RWC_UPDATE_REQUIRE_CHECKSUMS"
import json
import sys
...
require_manifest = sys.argv[4] == "1"
require_checksums = sys.argv[5] == "1"
...
if require_manifest and manifest is None:
    raise SystemExit("release is missing release-manifest.json")
PY
}
